...
- **Feedback Before Execution:** When the user proposes a new feature idea or asks for a major decision, DO NOT execute or write code immediately. You MUST first provide feedback, weigh the pros and cons (good vs bad), give a score based on the app's philosophy, and provide a clear recommendation.
...
## Project Overview
We are building **Antyo Finance**, a budgeting and personal finance app that bridges the gap between planning and reality. 
The app tracks planned budgets versus actual spending, displaying them side-by-side in a clear, insightful interface.

**Product Identity & Brand Core:**
Antyo Finance is not just a budget tracker. It is an identity mirror and decision system.
The central belief: **"Where your money goes, your future grows."**
The app helps users see who they are becoming based on their daily financial decisions. Money, budgets, and transactions are evidence of this. (Financial freedom and conscious spending serve as long-term supporting metaphors).

**Feature Decision Rule (CRITICAL):**
Before building or modifying any feature, it must answer: *"Does this feature help the user see who they are becoming financially or make a better identity-aligned financial decision?"*
...
- **Testing:** Unit tests are MANDATORY for the `/src/utils` folder (e.g., currency math, date logic). UI testing can wait for the MVP, but core financial calculation must be bulletproof.
...
/app             # Expo Router routes and screens only
  ...
  /(tabs)        # Main tab navigation (Dashboard, Budget, Transactions, Stats)
...
/src
  ...
  /store         # Zustand stores (useBudgetStore, useTransactionStore)
  /types         # Global TypeScript interfaces
  /utils         # Pure, testable functions (e.g., financial math, date logic)
...
- **Logic Separation:** Keep complex financial math and date calculations out of UI components. Extract them into `/src/utils`.

---

## UI Implementation Rules (VERY IMPORTANT)
- **From Scratch:** Create an original, premium, and unique implementation. The UI should not distract the user. Ensure BUDGET vs ACTUAL SPENDING is clearly distinguishable.
...
### Interactive UI & Navigation Context Rules (CRITICAL)
- **Conditional Shadows/Opacity:** If you create a button or tab that changes appearance when clicked, it is safe to conditionally change colors (`bg-blue-600`, `text-white`), but **NEVER** conditionally add `shadow-*`, `opacity-*`, or `transition-*` in the `className`. If they must change on click, put them in the `style={{}}` prop instead to prevent React Navigation "Couldn't find a navigation context" crashes.
- **Direct UI State Updates:** For normal buttons that only switch UI state (like tabs, timeframes, or segmented controls), use direct state updates (e.g., `onPress={() => setActiveTab('projects')}`). **DO NOT** wrap simple UI state updates in `InteractionManager.runAfterInteractions`.
...
- **The Re-render Trap:** Isolate rapid-updating state or heavy lists (like transaction history) from static UI state to prevent global re-renders. Use Zustand's atomic selectors or `useShallow`.
...
- **Future-proofing:** All core domain models (`Transaction`, `Budget`) MUST use UUIDs for identification to ensure seamless synchronization when a backend is eventually introduced.
